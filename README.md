...
In 1785, Coulomb assumed — without experimental proof — that electric force is proportional to the product of two charges. In 1820, Ampère derived a force law between current elements that satisfies Newton's third law exactly, acts along the line connecting them, and requires no field. In 1845, Grassmann proposed a replacement that violates Newton's third law at the element level. Maxwell encoded Grassmann's choice into his field equations in 1873. Every electromagnetic engineering tool built since — including SPICE, the simulation software behind every chip on Earth — inherited that choice invisibly. This presentation traces the dependency chain from Coulomb's assumed proportionality through to the semiconductor industry's 300+ empirical fitting parameters and the AI systems now learning those patches as if they were physics.

**Section 01: The Assumed Proportionality** — Coulomb demonstrated inverse-square distance dependence with his torsion balance but never proved the force is proportional to the product of charges qq′. He assumed it by analogy with Newton's gravity. Heering's 1992 replication found the data suspiciously clean for the instrument's actual precision. Gillmor's historical analysis confirmed the gap. The foundation of electrostatics rests on an analogy, not a measurement.

**Section 02: Ampère — The Newton of Electricity** — Within weeks of Ørsted's 1820 discovery, Ampère demonstrated wire-to-wire forces, proved current flows in closed circuits through the battery interior, and proposed all magnetism is caused by electric currents. His force law, derived from four null experiments, is central (along the connecting line) and satisfies Newton's third law exactly, element by element. Maxwell called it "one of the most brilliant achievements in science."

**Section 03: The Replacement — Grassmann and the Broken Law** — Grassmann's 1845 force law has components perpendicular to the connecting line and violates Newton's third law at the element level. For complete closed circuits, both laws integrate to identical results — the preference was philosophical, not experimental. Interactive canvas: toggle between Ampère's force (always along the line, always equal and opposite) and Grassmann's force (off-axis, unequal) with adjustable element geometry.

**Section 04: Biot-Savart's Three Errors** — Error 1: unjustified decomposition of a whole-wire measurement into element contributions (infinitely many element laws produce the same total). Error 2: assumed perpendicularity of the element force (hypothesis, not measurement). Error 3: force between a current element and a magnetic pole (mixing entities of different nature). Ampère insisted the fundamental force must act between entities of the same kind.

**Section 05: Maxwell Encodes the Choice** — Maxwell praised Ampère as "the Newton of electricity" and then built his theory on Grassmann's force and Faraday's field concept. The field propagates at c. Ampère's instantaneous action at a distance was replaced. The choice was institutional — the British school's field concept over the continental action-at-a-distance tradition — not experimental. Timeline from 1820 to the institutionalisation of Maxwell's equations as unchallengeable truth.

**Section 06: The Instantaneous Force Evidence** — Laplace calculated that for orbits to be stable, gravity must propagate at least 7 million times c. Van Flandern tightened this to 2 × 10¹⁰ × c. GR handles this by adding velocity-dependent correction terms that cancel the lag its own framework predicts — rebuilding by hand what instantaneous force gives for free. Miller's positive ether-drift result (326,000 interferometer turns, 5.2 million measurements, consistent 0.12 ± 0.01 fringe shift) and Einstein's conditional admission that relativity falls if Miller is right. The aether-drag hypothesis: if matter moves through a physical medium regardless of temperature, this motion — not thermal energy — sustains Ampère's molecular currents, explaining why magnetism peaks at absolute zero without invoking quantum labels. Testable prediction: sidereal-time variation in saturation magnetisation at millikelvin temperatures, measurable with SQUID magnetometry.

**Section 07: SPICE — Where the Wrong Theory Meets Silicon** — SPICE's device models derive from Maxwell's equations. They don't work. The BSIM4 model has 300+ empirical fitting parameters with no physical meaning, tuned to match fabricated silicon. Each foundry provides its own parameter set (PDK) under NDA. The corrections to the theory are the trade secret. Verification consumes over 70% of chip design time. At the 2nm GAA node, current flows in three-dimensional nanosheet structures — precisely the regime where Ampère and Grassmann diverge most — and the old fudge factors fail.

**Section 08: The Grind at Every Scale** — Four-panel comparison showing the identical pattern across EUV lithography (ASML's plasma models can't capture the behaviour, 0.02% efficiency, Lawrence Livermore supercomputers brought in), CNC precision (Japanese/German servo tuning as accumulated proprietary corrections), quantum computing (individually calibrated qubits, circular validation, Gil Kalai's mathematical objection), and supercolliders (instruments designed, calibrated, and analysed using the theory they test). Four domains, four scales, same structure: theory → anomaly → patch → lock-in.

**Section 09: The Nvidia Ceiling** — Engineers working seven days a week until 2 a.m., 76% millionaires, too exhausted to question anything fundamental. AI trained on empirical correction data learns the patches with increasing confidence, encoding the wrong framework deeper. The gradient descent trap: more data makes it worse if the data is generated within the wrong framework. The ceiling approaches from both directions — anomalies multiply as geometries shrink into the divergence regime, while AI makes the wrong theory ever harder to question.

**Section 10: The Exit** — Path 1: continue grinding (more parameters, more cost, rarer materials, converging timelines). Path 2: go back to Ampère (rebuild device physics from a force law that satisfies Newton's third law, see which parameters become unnecessary, see which anomalies disappear). The full vertical dependency chain from Coulomb's 1785 assumption to the AI ceiling. Nobody has funding for Path 2. The 3D ferrocell that could visually demonstrate actual magnetic field geometry costs less than one hour of CERN beam time and has never been built.

Related post: [The Logical Dependency Chain of I Finished Elementary](https://www.ifinishedelementary.com/post/the-logical-dependency-chain-of-i-finished-elementary)
...
## Testable Predictions from Chain 9

1. **Sidereal-time variation in saturation magnetisation** — Measure the saturation magnetisation of a ferromagnet (e.g. high-purity iron) at millikelvin temperatures using SQUID magnetometry, tracked continuously across months. If aether drag sustains Ampère's molecular currents, the saturation magnetisation should exhibit a parts-per-million periodic variation correlated with sidereal time — as Earth's rotation changes the laboratory's orientation relative to the aether drift direction Miller identified. Positive detection would simultaneously confirm Miller's drift, Ampère's molecular currents, and the aether-driven mechanism.
2. **Ampère vs Grassmann at the element level** — Design an experiment using short, transient current pulses in open conductor segments (not closed circuits) where Ampère's and Grassmann's force laws give different predictions. At the closed-circuit level both laws are indistinguishable; the divergence only appears for open current elements. Modern high-frequency electronics routinely operate in this regime — the experiment would require precision force measurement on short conductor segments carrying nanosecond-scale pulses.
3. **SPICE parameter reduction under Ampère** — Rederive the semiconductor device physics from Ampère's force law instead of Grassmann/Maxwell. Rebuild SPICE transistor models from this foundation. Determine how many of the 300+ BSIM4 empirical parameters become unnecessary — i.e. how many anomalies the Grassmann framework created that Ampère's framework resolves without fitting.
4. **3D ferrocell construction** — Build a volumetric magnetic field visualisation apparatus using stacked layers of optical glass with ferrofluid between each pair, a magnet suspended in the centre. Determine whether the three-dimensional field structure matches textbook flux line diagrams (continuous pole-to-pole arcs) or the toroidal geometry observed in 2D ferrocell experiments by Markoulakis et al. (two distinct toroidal bubbles at each pole with a Bloch wall boundary).
5. **Coulomb proportionality test** — Direct experimental verification of the proportionality of electric force to the product of charges qq′, using modern precision electrostatic apparatus. Coulomb's original claim rested on analogy with gravity, not measurement. A controlled experiment varying one charge while holding the other constant, across multiple charge magnitudes, would either confirm or deny the assumed proportionality with precision unavailable in 1785.
